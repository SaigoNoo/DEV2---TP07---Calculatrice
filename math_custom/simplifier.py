class Simplifier:
    def check_erreur(self, fract: tuple, operateur: str) -> int:
        ...
        :raise: Si le den = 0
        :raise: Si le num et le den sont pas des int
        :raise: Si le den = 1 et le num 0 et que l'operateur est dans [*, /]
        :parameter fract: La fraction attendue doit être de type tuple, et le contenu des tuples doivent être des integers.
        :parameter operateur:
        :return: 0 si tout est passé au niveau des tests
        """
        # Dénominateur à 0
        if fract[1] == 0:
    ...
    def gcd(self, num: int, den: int) -> int:
        ...
        **Reference**: Théoreme d'Euclide
        :param num: il s'agit du numérateur. Seul un integer est attendu
        :param den: il s'agit du dénominateur. Seul un integer est attendu
        :return: le facteur commun le plus grand entre le num et den
        """
        while den:
            num, den = den, num % den
        return num
    ...
    def simplifier_values(self, fraction: tuple) -> tuple:
        ...
        num et den.
        :param fraction: La fraction attendue doit être de type tuple, et le contenu des tuples doivent être des integers.
